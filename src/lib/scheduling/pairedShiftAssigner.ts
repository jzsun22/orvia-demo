...
/**
 * Handles the special case for the paired "Prep+Barista" shift in Cedar Loop.
...
export function processPairedPrepBaristaShifts(
    ...
    const template1 = templates.find(t => 
        t.location_id === CEDAR_LOOP_LOCATION_ID &&
    ...
    if (!template1 || !template2) {
        warnings.push("Could not find one or both required Prep+Barista paired shift templates for Cedar Loop. Skipping pair assignment.");
    ...
    for (const dayOfWeek of applicableDays) {
        ...
        if (!locationHours) {
             warnings.push(`Missing operating hours for ${dayOfWeek} at Cedar Loop. Cannot assign paired shift.`);
        ...
        const candidates = workers.filter(worker => {
            ...
            const hasLocation = workerData.locations?.some((wl: { location: Location }) => wl.location?.id === CEDAR_LOOP_LOCATION_ID) ?? false;
            if (!hasLocation) {
                console.log(`${logPrefix} REJECTED - Not assigned to Cedar Loop location. Checked workerData.locations.`);
                return false;
            }
